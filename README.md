...
bash
pnpm --filter ./artifacts/orca-marine-intelligence add -D \
...
Problem	Solution
PORT environment variable is required	Create .env file as described in Step 4, or run PORT=5173 BASE_PATH=/ pnpm dev
...
bash
rm -rf node_modules pnpm-lock.yaml   # Windows: use `rd /s node_modules` and delete pnpm-lock.yaml manually
pnpm store prune
pnpm install
